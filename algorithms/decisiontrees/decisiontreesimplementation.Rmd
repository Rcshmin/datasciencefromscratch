...
The main algorithm is a recursive function that calls the helpers to split the data, given that the stopping conditions have not been violated. We will input the data into this function, and it will return the tree. 
...
decision_tree_algorithm <- function(df, 
  ...
  if(any(check_purity(data), nrow(data) < min_samples, (counter - 1) == max_depth)){
   ...
   #Recursive part
    ...
    data_above = data_g[[1]]
    data_below = data_g[[2]]
    print(paste(split_column, split_value))
    #Find the answers
    yes_answer = decision_tree_algorithm(df = data_below, counter = counter + 1, min_samples, max_depth, splitvec)
    no_answer = decision_tree_algorithm(df = data_above, counter = counter + 1, min_samples, max_depth, splitve)
...
decision_tree <- function(df, min_samples, max_depth){
  ...
  decisions_split = as.data.frame(strsplit(decisions, split = " "))
  decisions_df = decisions_split[-1, ]
  decisions_df_clean = as.data.frame(sapply(decisions_df, function(x) gsub("\"", "", x)))
  decisions_df_t = as.data.frame(t(decisions_df_clean))
  
  return(decisions_df_t)
}


d11 = decision_tree(df = salmon_fish, min_samples = 10, max_depth = 10)

decisions = capture.output(decision_tree_algorithm(df = salmon_fish, max_depth = 4, min_samples = 10))

rtree <- list("Length < 3?", "Tuna", list("Weight < 4", "Salmon", list("Length > 6.75", "Tuna", "Salmon")))
...
decision_tree_algorithm(df = salmon_fish, min_samples = 10, max_depth = 3)

test_tree =  rpart(type ~ weight + length, data = salmon_fish, maxdepth = 3, parms = list(split = "information"))
