...
> Bias is the inherent error obtained from a model. It is due to our model being "biased" to a particular kind of solution. Bias is inherent to our model of choice. On the other hand, variance captures how much a model changes depending on what training set it is trained on. Underfitting and overfitting are two important terms here. Underfitting is when a model is too simple too accurately capture the relationships between the features and the output variable. An overfitted model is so specific to the data it was trained on, that it is no longer applicable to different datasets. 
