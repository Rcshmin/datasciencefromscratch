...
In terms of MSE, it seems that at $k=6$, MSE is the closes to zero. However, in terms of the MAE and MAPE, $k=10$ seems to be the best. This is as for $k=10$ the k-NN regressor is on average 0.14 units away from the true value, or around 19% away. One this is clear though. The k-NN algorithm performs much better in the classfication problem, then regression on the iris data set. 
