...
Before I close out this article, there is one interesting behavior of the k-NN algorithm in the regression problem, I want to bring to your attention. Well it not so much an interesting behavior but rather a cool visual attribute of the algorithm. First let us consider the iris data set in which we use the first column to predict the second column. That is, we use sepal length to predict speal width. 

```{r}
iris_proof = iris
knn_regress(df = iris_proof[, 1:2], k = 1, new_point = c(5))
